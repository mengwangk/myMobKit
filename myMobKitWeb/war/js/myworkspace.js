...
function maybeRequestTurn() {
  ...
  if (currentDomain.search('localhost') === -1 &&
      currentDomain.search('mymobkit') === -1) {
    // Not authorized domain. Try with default STUN instead.
    turnDone = true;
    return;
  }

	// Use numb.viagenie.ca TURN/STUN server

	// Create a turnUri using the polyfill (adapter.js).
	var iceServer = createIceServer('numb.viagenie.ca', 'mengwangk@gmail.com', 'Money123');
	if (iceServer !== null) {
...
function setLocalAndSendMessage(sessionDescription) {
  ...
  // Todo - Uncaught TypeError: Cannot read property 'setLocalDescription' of null
...
function sendMessage(message) {
	...
	// NOTE: AppRTCClient.java searches & parses this line; update there when
	// changing here.
	path = '/message?u=' + userEmail +'&w=' + workspaceId + '&sid=' + sessionId; 
...
function transitionToDone() {
	...
	remoteVideo.style.opacity = 0;
	miniVideo.style.opacity = 0;
	setStatus('You have left the call. <a href=' + workspaceLink + '>Click here</a> to rejoin.');
}
