 ...
 */
package com.mymobkit.mms.databases;
...
public class ApnDatabase {
    ...
    private ApnDatabase(final Context context) throws IOException {
        ...
        Utils.copy(context.getAssets().open(ASSET_PATH, AssetManager.ACCESS_STREAMING),
                new FileOutputStream(dbFile));

        try {
            this.db = SQLiteDatabase.openDatabase(context.getDatabasePath(DATABASE_NAME).getPath(), null,
    ...
    public LegacyMmsConnection.Apn getDefaultApnParameters(String mccmnc, String apn) {
        ...
        try {
            ...
            while (cursor != null && cursor.moveToNext()) {
                ...
                if (isApnValid) {
                    LegacyMmsConnection.Apn params = new LegacyMmsConnection.Apn(cursor.getString(cursor.getColumnIndexOrThrow(MMSC_COLUMN)),
    ...
    public LegacyMmsConnection.Apn getMmsConnectionParameters(String mccmnc, String apn) {
        final LegacyMmsConnection.Apn customApn = TelephonyUtils.getConfiguredApn(context);
        final LegacyMmsConnection.Apn defaultApn = getDefaultApnParameters(mccmnc, apn);

        if (!TextUtils.isEmpty(customApn.getMmsc()))
            return customApn;
        else {
