 ...
 *
 */
class AliasTable extends DatabaseTable {
    
    public AliasTable(Context ctx) {
        super(ctx);
    }
    ...
    public static boolean addAlias(String aliasName, String number, String contactName) {
        ...
        long ret = dbInstance.insert(DatabaseUtils.ALIAS_TABLE_NAME, null, values);
        return ret != -1;
    }
    ...
    public static boolean updateAlias(String aliasName, String number, String contactName) {
        ContentValues values = composeValues(aliasName, number, contactName);
        int ret = dbInstance.update(DatabaseUtils.ALIAS_TABLE_NAME, values, "aliasName='" + aliasName + "'", null);
        return ret == 1;
    }
    
    public static boolean deleteAlias(String aliasName) {
        int ret = dbInstance.delete(DatabaseUtils.ALIAS_TABLE_NAME, "aliasName='" + aliasName + "'", null);
        return ret == 1;
    }
    
    public static String getNumber(String aliasName) {
        Cursor c = dbInstanceReadOnly.query(DatabaseUtils.ALIAS_TABLE_NAME, new String[] { "number" }, "aliasName='" + aliasName + "'", null, null , null, null);
        if(c.getCount() == 1) {
            c.moveToFirst();
    ...
    public static String[][] getFullDatabase() {
        ...
        Cursor c = dbInstanceReadOnly.query(DatabaseUtils.ALIAS_TABLE_NAME, new String[] { "aliasName", "number", "contactName" }, null, null, null , null, null);
        if (c != null) {
    ...
    public static String[] getAlias(String aliasName) {
        String[] res = null;
        Cursor c = dbInstanceReadOnly.query(DatabaseUtils.ALIAS_TABLE_NAME, new String[] { "aliasName", "number", "contactName" }, "aliasName='" + aliasName + "'", null, null , null, null);
        if (c != null) {
            c.moveToFirst();
