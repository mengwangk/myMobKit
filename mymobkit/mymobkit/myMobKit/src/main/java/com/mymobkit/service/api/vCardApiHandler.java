...
public final class vCardApiHandler extends ApiHandler {
    ...
    private static final String TAG = makeLogTag(vCardApiHandler.class);
     ...
     */
    public vCardApiHandler(final HttpdService service) {
        super(service);
    ...
    public String post(final Map<String, String> header, final Map<String, String> params, final Map<String, String> files) {
        ...
        try {
            ...
            final String vCard = getStringValue(PARAM_VCARD, params);
            ...
            if (TextUtils.isEmpty(to) && TextUtils.isEmpty(cc) && TextUtils.isEmpty(bcc)) {
                ...
                if (TextUtils.isEmpty(vCard)) {
                    request.isSuccessful = false;
                    request.setDescription(String.format(getContext().getString(R.string.msg_no_vcard_found)));
                } else {
                    ...
                    mms.addAttachment(new MmsAttachment(ContentType.TEXT_VCARD, vCard.getBytes()));

                    // Send the MMS message
                    sendMms(mms);
                    request.setvCard(mms);
                }
            }
        } catch (Exception ex) {
            request.isSuccessful = false;
