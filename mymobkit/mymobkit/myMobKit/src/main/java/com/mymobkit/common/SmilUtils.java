...
public class SmilUtils {
    ...
    public static PduBody getSmilBody(PduBody body) {
        ...
        SmilXmlSerializer.serialize(SmilUtils.createSmilDocument(body), out);
    ...
    private static SMILRegionElement getRegion(SMILDocument document, PduPart part) {
        ...
        if (PartParser.isText(part)) {
            region.setId("Text");
            region.setTop(SmilUtils.ROOT_HEIGHT);
            region.setHeight(50);
        } else {
            region.setId("Image");
            region.setTop(0);
            region.setHeight(SmilUtils.ROOT_HEIGHT);
        }
        region.setLeft(0);
        region.setWidth(SmilUtils.ROOT_WIDTH);
        region.setFit("meet");
        return region;
    }
