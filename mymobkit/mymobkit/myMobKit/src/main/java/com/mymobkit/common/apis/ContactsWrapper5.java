...
public final class ContactsWrapper5 extends ContactsWrapper {
	/** Tag for output. */
	private static final String TAG = makeLogTag(ContactsWrapper5.class);
	...
	public Cursor getContact(final ContentResolver cr, final String number) {
		...
		if (c != null && c.moveToFirst()) {
			...
			if (c0 != null && c0.moveToFirst()) {
				LOGD(TAG, "id: " + c0.getString(FILTER_INDEX_ID));
				LOGD(TAG, "name: " + c0.getString(FILTER_INDEX_NAME));
				LOGD(TAG, "number: " + c0.getString(FILTER_INDEX_NUMBER));
				return c0;
			}
		}
		return null;
	...
	public boolean updateContactDetails(final Context context, final boolean loadOnly,
		...
		if (contact == null) {
			LOGW(TAG, "updateContactDetails(null)");
			return false;
		}
		LOGD(TAG, "updateContactDetails(" + contact.mRecipientId + ")");
		LOGD(TAG, "id: " + contact.mPersonId);
		LOGD(TAG, "number: " + contact.mNumber);
		LOGD(TAG, "name: " + contact.mName);
	...
	private byte[] loadAvatarData(final Context context, final Contact contact) {
		...
		try {
			...
			LOGE(TAG, "error recoding stream", e);
		} finally {
			try {
				if (avatarDataStream != null) {
				...
				LOGE(TAG, "error closing stream", e);
			}
		}
