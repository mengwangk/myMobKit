...
public class ContactsManager {
	...
	private static final String TAG = makeLogTag(ContactsManager.class);
	...
	public static String getContactNameOrNull(Context ctx, String phoneNumber) {
		...
		try {
			...
			if (c != null) {
				if (c.moveToFirst()) {
					res = ValidationUtils.getString(c, CommonDataKinds.Phone.DISPLAY_NAME);
				}
				c.close();
			}
		} catch (Exception ex) {
	...
	/**
	 * Tries to get the contact's display name of the specified phone number. If not found, returns the argument. If there is an error or phoneNumber is
	 * null, R.string.chat_call_hidden will be returned.
	 */
	public static String getContactName(Context ctx, String phoneNumber) {
		...
		if (phoneNumber != null) {
			try {
				...
				LOGE(TAG, "getContactName error: Phone number = " + phoneNumber, ex);
	...
	/**
	 * Tries to get the contact display name of the specified phone number. If not contact could be found null is returned.
	 */
	public static String getContactName(Context ctx, long rawId) {
		String res = null;

		ContentResolver resolver = ctx.getContentResolver();
		Cursor c = resolver.query(RawContacts.CONTENT_URI, new String[] { RawContacts.CONTACT_ID }, RawContacts._ID + "=?", new String[] { String.valueOf(rawId) }, null);

		long id = -1;
		if (c != null) {
			if (c.moveToFirst()) {
				id = ValidationUtils.getLong(c, RawContacts.CONTACT_ID);
			}
			c.close();
		}

		c = resolver.query(Contacts.CONTENT_URI, new String[] { Contacts.DISPLAY_NAME }, RawContacts._ID + "=?", new String[] { String.valueOf(id) }, null);

		if (c != null) {
			if (c.moveToFirst()) {
				res = ValidationUtils.getString(c, Contacts.DISPLAY_NAME);
			}
			c.close();
		}
	 ...
	 *            Application context
	 * @param searchedName
	 *            - the name of the contact or a phone number
	...
	private static ArrayList<Contact> getMatchingContacts(Context ctx, String searchedName, boolean isNameOnly) {
		...
		Uri contactUri = searchedName.equals("") ? Contacts.CONTENT_URI : Uri.withAppendedPath(Contacts.CONTENT_FILTER_URI, ValidationUtils.encodeSql(searchedName));
		Cursor c = resolver.query(contactUri, projection, null, null, sortOrder);
		if (c != null) {
			for (boolean hasData = c.moveToFirst(); hasData; hasData = c.moveToNext()) {
				Long id = ValidationUtils.getLong(c, Contacts._ID);
				String contactName = ValidationUtils.getString(c, Contacts.DISPLAY_NAME);
				...
				contact.ids.add(id);
				Cursor c1 = resolver.query(RawContacts.CONTENT_URI, new String[] { RawContacts._ID }, RawContacts.CONTACT_ID + "=?", new String[] { String.valueOf(id) }, null);
				if (c1 != null) {
					for (boolean hasData1 = c1.moveToFirst(); hasData1; hasData1 = c1.moveToNext()) {
						contact.rawIds.add(ValidationUtils.getLong(c1, RawContacts._ID));
					}
					c1.close();
				}
			}
	...
	public static ArrayList<ContactAddress> getPostalAddresses(Context ctx, ArrayList<Long> ids) {
		...
		if (ids.size() > 0) {
			...
			Cursor c = ctx.getContentResolver().query(ContactsContract.Data.CONTENT_URI, null, where, whereParams, null);
			if (c != null) {
				while (c.moveToNext()) {
					int type = ValidationUtils.getLong(c, ContactsContract.CommonDataKinds.StructuredPostal.TYPE).intValue();
					String label = ValidationUtils.getString(c, ContactsContract.CommonDataKinds.StructuredPostal.LABEL);
					...
					a.address = ValidationUtils.getString(c, ContactsContract.CommonDataKinds.StructuredPostal.DATA);
					a.label = label;
					res.add(a);
				}
				c.close();
	...
	/**
	 * Returns a ArrayList of <ContactAddress> containing email addresses which match to contact id You usually want to use
	 * ContactsResolver.resolveContact()
	 * 
	 * @param ctx
	 * @param ids
	 * @return
	...
	public static ArrayList<ContactAddress> getEmailAddresses(Context ctx, ArrayList<Long> ids) {
		...
		if (ids.size() > 0) {
			...
			if (c != null) {
				while (c.moveToNext()) {
					String label = ValidationUtils.getString(c, ContactsContract.CommonDataKinds.Email.LABEL);
					int type = ValidationUtils.getLong(c, ContactsContract.CommonDataKinds.Email.TYPE).intValue();
					...
					ContactAddress a = new ContactAddress();
					a.address = ValidationUtils.getString(c, ContactsContract.CommonDataKinds.Email.DATA);
					a.label = label;
					res.add(a);
				}
				c.close();
	 ...
	 *            Context
	 * @param ids
	 *            An ArrayList with the contact IDs
	...
	public static ArrayList<Phone> getPhones(Context ctx, ArrayList<Long> ids) {
		...
		if (ids.size() > 0) {
			...
			if (c != null) {
				while (c.moveToNext()) {
					String number = ValidationUtils.getString(c, CommonDataKinds.Phone.NUMBER);
					String label = ValidationUtils.getString(c, CommonDataKinds.Phone.LABEL);
					int type = ValidationUtils.getLong(c, CommonDataKinds.Phone.TYPE).intValue();
					int super_primary = ValidationUtils.getInt(c, ContactsContract.Data.IS_SUPER_PRIMARY);
					...
					if (phones.add(phone.getCleanNumber())) {
						...
						LOGI(TAG, "getPhones(ids) Duplicated phone number: " + number);
					}
				}
				c.close();
			}
	...
	/**
	 * Search for all phones with owners containing searchedText You usually want to use ContactsResolver.resolveContact()
	 * 
	 * @param ctx
	 * @param searchedText
				...
				for (Contact contact : contacts) {
					...
					for (Phone phone : phones) {
						...
						if (resPhones.add(phone.getCleanNumber())) {
							...
							LOGI(TAG, "getPhones(searchedText): Duplicated phone number: " + phone.getContactName() + " " + phone.getCleanNumber());
						}
					}
				}
			}
	...
	/**
	 * Returns a ArrayList<Phone> with all matching mobile phone for the argument You usually want to use ContactsResolver.resolveContact()
	 * 
	 * @param ctx
	 * @param searchedText
	 * @return ArrayList<Phone>
