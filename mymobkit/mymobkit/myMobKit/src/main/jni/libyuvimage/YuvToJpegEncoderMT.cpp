...
boolean YuvToJpegEncoderMT_encode(JNIEnv* env, jobject jstream, jbyteArray jstorage, void* inYuv, int width,
	...
	//jerr_arr = (struct mt_jpeg_error_mgr *)malloc(sizeof(struct mt_jpeg_error_mgr));
	//outbuffer_arr = (unsigned char**)malloc(sizeof(unsigned char *));
	//outsize_arr = (unsigned long*)malloc(sizeof(unsigned long));
	...
	//{
	    ...
	    cInfo.err = jpeg_std_error(&jerr_arr);
	    jerr_arr.error_exit = mt_jpeg_error_exit;
	    jerr_arr.thread_num = i+1;
	    if (err_thread_num = setjmp(jerr_arr.fJmpBuf)) {
	        err = true;
	        //break;
	    }
	    ...
	    YuvToJpegEncoderMT_setJpegCompressStruct(&cInfo, width, height, jpegQuality);

	    outsize_arr = bufsize;

	    outbuffer_arr = (uint8_t*)malloc(outsize_arr);

	    jpeg_mem_dest(&cInfo, &outbuffer_arr, &outsize_arr);

	    cInfo.restart_in_rows = thread_height/lines_per_iMCU_row;
    ...
    //i = 0;
	for (processed_lines = 0; processed_lines < height; processed_lines += thread_height)
	{
		boolean last_iter = (processed_lines + thread_height) >= height;
...
//#pragma omp parallel for num_threads(thread_num)
		...
		//{
			...
			if (fFormat == ImageFormat_NV21)
				err |= Yuv420SpToJpegEncoderMT_compress(&cInfo,
						(uint8_t*) inYuv, offsets, start_row, end_row, call_pass_startup);
			else
				err |= Yuv422IToJpegEncoderMT_compress(&cInfo, (uint8_t*) inYuv,
